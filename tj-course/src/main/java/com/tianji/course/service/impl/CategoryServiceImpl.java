...
public class CategoryServiceImpl extends ServiceImpl<CategoryMapper, Category> implements ICategoryService {
    ...
    public List<CategoryVO> list(CategoryListDTO categoryListDTO) {
        ...
        // 1.搜索条件根据priority正序排序，id逆序排序，这里生成的查询条件
        ...
        Map<Long, Integer> cateIdAndNumMap = courseService.countCourseNumOfCategory();
        ...
        List<CategoryVO> categoryVOS = TreeDataUtils.parseToTree(list, CategoryVO.class,
                ...
                (category, categoryVO) -> {
                    ...
                    categoryVO.setThirdCategoryNum(
                            NumberUtils.null2Zero(thirdCategoryNumMap.get(category.getId())).intValue());
    ...
    public List<SimpleCategoryVO> all(Boolean admin) {
        // 1.查询有课程的课程分类id列表
        List<Long> categoryIdList = admin ? null : courseService.getCategoryIdListWithCourse();
        // 1.1.判空
    ...
    private List<CategoryVO> fiter(List<CategoryVO> categoryVOList, CategoryListDTO categoryListDTO) {
        ...
        return categoryVOList.stream().filter(categoryVO -> filter(categoryVO, categoryListDTO))
                .collect(Collectors.toList());
    }
    ...
    private boolean filter(CategoryVO categoryVO, CategoryListDTO categoryListDTO) {
        ...
        if (pass && StringUtils.isNotEmpty(categoryListDTO.getName())) {// 状态pass通过后校验名称，包含名称关键字 通过
            pass = StringUtils.isNotEmpty(categoryVO.getName())
                    && categoryVO.getName().contains(categoryListDTO.getName());
        }
