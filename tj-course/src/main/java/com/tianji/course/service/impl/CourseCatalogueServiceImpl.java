 ...
 */
@Service
public class CourseCatalogueServiceImpl extends ServiceImpl<CourseCatalogueMapper, CourseCatalogue>
        implements ICourseCatalogueService {
    ...
    public List<CatalogueDTO> queryCourseCatalogues(Long courseId, Boolean withPractice) {
        // 1.课程目录查询条件
        LambdaQueryWrapper<CourseCatalogue> queryWrapper = Wrappers.lambdaQuery(CourseCatalogue.class)
        ...
        Map<Long, Long> cataIdAndNumMap = CollUtils.isEmpty(questionBizDTOS)
                ? new HashMap<>()
                : questionBizDTOS
                        .stream()
    ...
    public SectionInfoDTO getSimpleSectionInfo(Long sectionId) {
        ...
        sectionInfoDTO.setFreeDuration(
                courseCatalogue.getTrailer() == 1 ? courseProperties.getMedia().getTrailerDuration() : 0);
    ...
    public List<CataSimpleInfoVO> getCatasIndexList(Long courseId) {
        ...
        // 2.章id与章序号映射关系
        Map<Long, Integer> chapterMap = courseCatalogues
                .stream()
    ...
    public List<CataSimpleInfoVO> getManyCataSimpleInfo(List<Long> ids) {
        ...
        // 2.查询条件,这里生成的只是条件，这里可以使用 listByIds
        LambdaQueryWrapper<CourseCatalogue> queryWrapper = Wrappers.lambdaQuery(CourseCatalogue.class)
    ...
    public List<CataVO> queryCourseCataloguesVO(Long courseId, Boolean withPractice) {
        ...
        // 6.数据目录结构转化
        List<CataVO> cataVOS = TreeDataUtils.parseToTree(courseCatalogues, CataVO.class,
    ...
    // 课程目录树形转化模型
    private static class CourseCatalogDataWrapper
            implements TreeDataUtils.DataProcessor<CatalogueDTO, CourseCatalogue> {
